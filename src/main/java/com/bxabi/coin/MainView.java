...
public class MainView extends VerticalLayout {
    ...
    private void addFooter() {
        ...
        footer.add(new Html("<p>Icon made by <a target='_blank' href=\"https://www.freepik.com/\" title=\"Freepik\">Freepik</a> from <a target='_blank' href=\"https://www.flaticon.com/\""
            + "      title=\"Flaticon\">www.flaticon.com</a> is licensed by <a href=\"http://creativecommons.org/licenses/by/3.0/\"       " + "          title=\"Creative Commons BY 3.0\" target=\"_blank\">CC 3.0 BY</a></p>"));
        add(footer);
    ...
    private NumberField addRow(String coinId, Double value) {
        ...
        // c1.setPreventInvalidInput(true);
        c1.setAutoOpen(true);
        combos.add(c1);
    ...
    private class NumberChangeListener implements ValueChangeListener<ComponentValueChangeEvent<? extends Component, ?>> {
    ...
    private class CoinChangeListener implements ValueChangeListener<ComponentValueChangeEvent<? extends Component, ?>> {
        ...
        public void valueChanged(ComponentValueChangeEvent<? extends Component, ?> event) {
            ...
            if (value != null) {
                coinIds.set(id, event.getValue()
                    .toString());
                priceService.refreshPrices(coinIds, true);
            } else
                coinIds.set(id, null);
            calculateValues();
        }
    ...
    private void calculateValues() {
        boolean success = priceService.refreshPrices(coinIds, false);
        if (success) {
            lastPriceUpdate.setText("Last price update: " + DATEFORMAT.format(priceService.getLastUpdated()));
        } else {
            lastPriceUpdate.setText("The latest price update request failed, one ore more coins may have no price or an outdated one!");
        }
        // lastPriceUpdate.setClassName("red", !success);

        Double value = numbers.get(activeRow)
            .getValue();
        if (value == null) {
            return;
        }
        ...
        for (int i = 0; i < numbers.size(); i++) {
            ...
            if (to != null) {
                ...
                numbers.get(i)
                    .setValue(converted.doubleValue());
            }
        }
    }
}
